...
> 	&= (0, 0). \\
> \end{align}$$
> If $\char \Z_{n} \oplus \Z_{m} < \lcm (n,m)$, then it is not a [[Divisibility|multiple]] of $n$ or $m$, meaning $n \char \Z_{n} \oplus \Z_{m} = 0$ or $m \char \Z_{n} \oplus \Z_{m} = 0$.
...
> Then since $1n = 0$, $ab = 0$, but $a$ and $b$ are non-zero, because this would violate the minimality of $n$.

...
> \end{document}
> ```
> - and [[Composite Functions|composition]] being the same as in $C$.
 ^auxiliary-category
...
By the [[Initial and Terminal Objects|definition of terminal objects]], for any $\left( x, \{\alpha_i\}_{i \in I} \right) \in D$, there exists a unique $\gamma : x \to p$ in $C$ such that $\forall i, \pi_i\gamma = \alpha_i$, i.e., such that the following diagram is [[Commutative Diagrams|commutative]] for all $i$:
```tikz
\usepackage{tikz-cd}
\begin{document}
...
> Objects $a,b \in C$ have a *product* $a \times b$ and $\pi_1,\pi_2$ iff for every $x \in C$, there exists a unique morphism $f : x \to (a \times b)$ such that the above diagram is [[Commutative Diagrams|commutative]] for all $f_1$ and $f_2$.

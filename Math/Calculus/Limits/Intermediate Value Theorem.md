...
Suppose $f(x)$ is [[Metric Continuity 1|continuous]] on $\left[a,b\right]$. The *intermediate value theorem* states that for every $y$ in $\left[f(a),f(b)\right]$ or $\left[ f(b),f(a) \right]$, there exists an input in $[a,b]$ that maps to it. 
$$\begin{align}
...
> Both $\left( a_{n} \right)$ and $\left( b_{n} \right)$ [[Metric Convergence|converge]] by [[Monotone Convergence Theorem|monotone convergence theorem]] to $c$, so we can find $N$ for which for all $n \ge N$,
> $$\begin{array}{c}

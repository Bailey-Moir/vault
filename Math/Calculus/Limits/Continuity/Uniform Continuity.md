...
> [!note] This is the same as regular [[Metric Continuity 1|continuity]], except the $a$ is given after the $\delta$ instead of before the $\varepsilon$: $\mathbf{\delta}$ **cannot depend on** $\mathbf{a}$.
...
> 
> We first check what happens at the midpoint $\frac{a_{n}+b_{n}}{2}$. Since $f$ is [[Metric Continuity 1|continuous]] at this point, there exists $\delta > 0$ such that for $\left| z - \frac{a_{n}+b_{n}}{2} \right| < \delta$, $\left| f(z) - f(\frac{a_{n}+b_{n}}{2}) \right| < \varepsilon$.
> 
...
> 
> But $f$ is [[Metric Continuity 1|continuous]]  on $\left[ a,b \right]$, so $\lim f(x_{n}) = \lim f(y_{n}) = f(x)$[^3], but we know that $\left| f(x_{n}) - f(y_{n}) \right| \ge \varepsilon$ for all $n$, which this [[Contradictions|contradicts]].

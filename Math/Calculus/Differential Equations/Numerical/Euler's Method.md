...
> for some $c \in \left( t_{0},t_{1} \right)$ (note $t_{1} = t_{0}+h$). Suppose $y''(t)$ is [[Metric Continuity|continuous]] on $\left[ t_{0},t_{1} \right]$. Then it is [[Bounded Sets|bounded]][^1], so let $\left| y'' \right| \leq M$ for some $M$ on $(t_{0},t_{1})$. Then
> $$\begin{align}

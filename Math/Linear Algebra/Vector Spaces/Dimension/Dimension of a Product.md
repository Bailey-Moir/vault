...
> [!proof]-
> Choose a [[Vector Basis|basis]] for each $V_{k}$. For each [[Vector Basis|basis vector]] $\vecbf{v}$ of each $V_{k}$, consider
> $$\begin{array}{cccccc}
> 	(\vecbf{0}, &\cdots, &\vecbf{v}, &\cdots, &\vecbf{0}) &\hspace{-0.8em}\in V_{1}\times \dots \times V_{n} \\
> 	\tiny 0\text{-th} && \tiny k\text{-th} && \tiny n\text{-th} &\\
> \end{array}$$
> These vectors are [[Linear Independence|linearly independent]] and [[Span|span]] $V_{1}\times \dots \times V_{n}$. Thus, this is a [[Vector Basis|basis]] of length $\text{dim}\,V_{1} + \dots + \text{dim}\,V_{n}$.
